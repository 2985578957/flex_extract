...
def install_via_gateway(c):
    ...
    tar_file = os.path.join(ecd + os.path.sep + tarball_name)
    ...
    if c.install_target.lower() != 'local': # ecgate or cca
        ...
        submit_job_to_ecserver(c.install_target,
                               os.path.join(_config.PATH_JOBSCRIPTS +
                                            os.path.sep +
        ...
        if not c.flexpart_root_scripts or c.flexpart_root_scripts == '../':
            #install_dir = c.flexpart_root_scripts
            print('WARNING: FLEXPART_ROOT_SCRIPTS has not been specified')
            print('There will be only the compilation of the Fortran program' +
                  ' in ' + _config.PATH_FORTRAN_SRC)
            ...
            if os.path.abspath(ecd) != os.path.abspath(c.flexpart_root_scripts):
                mk_tarball(tar_file)
                make_dir(os.path.join(c.flexpart_root_scripts + os.path.sep +
                                      target_dirname))
                os.chdir(os.path.join(c.flexpart_root_scripts + os.path.sep +
                                      target_dirname))
                un_tarball(tar_file)
                os.chdir(os.path.join(c.flexpart_root_scripts + os.path.sep +
                                      target_dirname + os.path.sep +
...
def mk_tarball(tarball_path):
    ...
    ECMWF_ENV_FILE = [os.path.join(_config.PATH_RELATIVE_PYTHON +
                                   os.path.sep + _config.FILE_USER_ENVVARS)]
...
def mk_env_vars(ecuid, ecgid, gateway, destination):
    ...
    with open(os.path.join(_config.PATH_LOCAL_PYTHON + os.path.sep +
                           _config.FILE_USER_ENVVARS), 'w') as fo:
...
def mk_compilejob(makefile, target, ecuid, ecgid, fp_root):
    ...
    template = os.path.join(_config.PATH_TEMPLATES + os.path.sep +
                            _config.TEMPFILE_INSTALL_COMPILEJOB)
    with open(template) as f:
        fdata = f.read().split('\n')

    compilejob = os.path.join(_config.PATH_JOBSCRIPTS + os.path.sep +
                              _config.FILE_INSTALL_COMPILEJOB)
...
def mk_job_template(ecuid, ecgid, gateway, destination, fp_root):
    ...
    @Return:
        <nothing>
    '''
    ec_python_rel_path = _config.FLEXEXTRACT_DIRNAME + '/' + \
    ...
    jobfile_temp = os.path.join(_config.PATH_JOBSCRIPTS + os.path.sep +
    ...
    with open(jobfile_temp, 'w') as fo:
        for data in fdata:
            if '--workdir' in data:
                ...
                data += fp_root + '/' + ec_python_rel_path
...
def make_convert_build(src_path, makefile):
    ...
    try:
        ...
        p = subprocess.Popen(['make', '-f',
                              os.path.join(src_path + os.path.sep + makefile)],
        ...
        subprocess.check_call(['ls', '-l',
                               os.path.join(src_path + os.path.sep +
